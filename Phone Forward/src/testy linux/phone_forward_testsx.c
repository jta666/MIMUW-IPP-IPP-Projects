...
static int cycle(void) {
    ...
    do {
        PhoneNumbers *_p;
        printf("a \n");
        //N(_p = phfwdGetReverse(pf, "123"));
        do {
            if ((_p = phfwdGetReverse(pf, "123")) == 0)
                return FAIL;
        } while (0);
        printf ("XXXXXXXXXXXXXXXXXX");
        char const * xxf =  phnumGet(_p, 0);
        ...
        for (size_t _k = 0; _k < SIZE(_r); ++_k) {
            //R(_p, _k, _r[_k]);
            do {
                printf("mkmkmkmkmkmkmkmmkmkkmkm");
                if (strcmp(phnumGet(_p, _k), _r[_k]) != 0)
                    return FAIL;
            } while (0);
        }
        printf("d \n");
...
// Realokacja musi się udać, jeśli nie zwiększamy rozmiaru alokowanej pamięci.
 bool can_fail(void const *old_ptr, size_t new_size) {
  if (old_ptr == NULL)
    return true;
  else
