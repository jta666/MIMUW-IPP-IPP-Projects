...
PhoneNumbers* createPhoneNumber(char const* num) {
    ...
    if (isNotPhoneNumber(num)) return NULL;
...
void phfwdRemove(PhoneForward* pf, char const* num) { phfwdRemove_(pf, NULL, num); }
...
PhoneNumbers* phfwdGet_(PhoneForward const* pf, char const* num, Redirection* best) {
    ...
    if (walker->original) {  // sprawdzamy, czy jest przekierowanie
...
char const* phnumGet(PhoneNumbers const* pnum, size_t idx) {
    ...
    PhoneNumbers* walker = pnum->next;  // pnum to zawsze pusta głowa
...
/**
 * Pomocnicza funkcja zwracająca liczbę, która optymalnie roszerza pamięć.
 */
 ...
 */
void phfwdReverse_(PhoneForward const* curr, char const* toCheck, char*** silo, int* currSize, int* maxSize,
                   char const* num) {
    if (!curr) return;

    if (curr->nextCurr) phfwdReverse_(curr->nextCurr, toCheck, silo, currSize, maxSize, num);
    if (curr->nextNext) phfwdReverse_(curr->nextNext, toCheck, silo, currSize, maxSize, num);

    if (!silo) return;

    if (curr->redirected && !strcmp(curr->redirected, toCheck)) {
        ...
        int k = 0;
        for (int i = 0; i < orgLen; i++) newNum[k++] = curr->original[i];
        for (int i = redLen; i < numLen; i++) newNum[k++] = num[i];
        newNum[k] = '\0';
        ...
        for (int i = 0; i < *currSize; i++) {
            if (!strcmp(newNum, (*silo)[i])) found = true;
        }
...
/**
 * Pomocnicza funkcja do porównywania numerów w wymagany przez specyfikację sposób.
 */
int strcmpButBetter(const char* s1, const char* s2) {
    while (*s1 && (*s1 == *s2)) {
        s1++;
        s2++;
    }
 ...
 */
int cmpStr(void const* a, void const* b) { return strcmpButBetter(*((char**)a), *((char**)b)); }
...
PhoneNumbers* phfwdReverse(PhoneForward const* pf, char const* num) {
    ...
    for (int i = 0; i < numLen; i++) {
        ...
        prefix[i + 1] = '\0';
        for (int j = 0; j <= i; j++) prefix[j] = num[j];
    ...
    if (numSilo) qsort(numSilo, *currLen, sizeof(*numSilo), cmpStr);
    ...
    if (numSilo) {
        for (int i = 0; i < *currLen; i++) free(numSilo[i]);
        free(numSilo);
    }
...
bool contains(PhoneNumbers* const pnum, char const* num) {
    if (!pnum) return false;

    int i = 0;
    while (phnumGet(pnum, i)) {
        if (!strcmp(phnumGet(pnum, i), num)) return true;
        i++;
    }
...
PhoneNumbers* phfwdGetReverse(PhoneForward const* pf, char const* num) {
    ...
    while (walker) {
        if (!(walker->num)) {
            ...
            char* s = walker->num;
            PhoneNumbers* check = phfwdGet(pf, s);
            if (!contains(check, num)) {
                phnumDelete(check);
                walkerPrev->next = walker->next;
                PhoneNumbers* temp = walker;
                walker = walker->next;
                free(temp->num);
                free(temp);
                temp = NULL;
